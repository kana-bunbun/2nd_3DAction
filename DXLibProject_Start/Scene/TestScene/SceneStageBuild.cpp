...
#include "World/UI/Screen/StageBuildScreen.h"
...
SceneStageBuild::SceneStageBuild() :
	m_pScreenManager(nullptr)
{
	m_pScreenManager = std::make_unique<ScreenManager>();
	m_pScreenManager->PushScreen(std::make_unique<StageBuildScreen>());

}

SceneStageBuild::~SceneStageBuild() {}

void SceneStageBuild::Init() {
...
std::unique_ptr<SceneBase> SceneStageBuild::Update(float deltaTime) {
	ProfileScope("Update");
...
void SceneStageBuild::DrawGround()
{
	Vector3 pos1;
	Vector3 pos2;
